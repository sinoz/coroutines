...
import com.storm_enroute.coroutines.common.Cache
import com.storm_enroute.coroutines.common._
import com.storm_enroute.coroutines.common.Cache._
...
trait CfgGenerator[C <: Context] {
  ...
  abstract class Node {
    ...
    protected def genSaveState(subgraph: SubCfg)(implicit t: Table): List[Tree] = {
      ...
      val pcstackset = q"""
        _root_.com.storm_enroute.coroutines.common.Stack.update($cparam.$$pcstack, $pc.toShort)
      """
    ...
    protected def genExit(value: Tree, subgraph: SubCfg)(implicit t: Table): Tree = {
      ...
      q"""
        $$pop($cparam)
        if (_root_.com.storm_enroute.coroutines.common.Stack.isEmpty($cparam.$$costack)) {
          $$assignresult($cparam, $untypedtree)
        } else {
          $cparam.$$target = $cparam
          _root_.com.storm_enroute.coroutines.common.Stack.top($cparam.$$costack)
      ...
      def genCoroutineCall(
        ...
        q"""
          ...
          $untypedco.$$push[..$tpargs](
            $cparam.asInstanceOf[
              _root_.com.storm_enroute.coroutines.Coroutine.Instance[$yldtpe, $rettpe]],
            ..$untypedargs)
          $cparam.$$target = $cparam
          return
        """
    ...
    case class YieldTo(
      ...
      def emit(z: Zipper, seen: mutable.Set[Node], subgraph: SubCfg)(
        ...
        val exittree = q"""
          ..$savestate
          if (!$untypedco.isLive)
            throw new _root_.com.storm_enroute.coroutines.CoroutineStoppedException
          $cparam.$$target =
