...
import com.storm_enroute.coroutines.common._
...
with AstCanonicalization[C] {
    ...
    val defdef = if (subgraph.uid < NUM_PREDEFINED_ENTRY_STUBS) q"""
      override def $defname(
        ${t.names.coroutineParam}:
          _root_.com.storm_enroute.coroutines.Coroutine.Instance[${t.yieldType}, ${t.returnType}]
      ): _root_.scala.Unit = {
        $body
      }
    """ else q"""
  ...
  private def genEnterMethod(entrypoints: Map[Long, Tree])(
    ...
    if (entrypoints.size == 1) {
      ...
      q"""
        def $$enter(
          c: _root_.com.storm_enroute.coroutines.Coroutine.Instance[$yldtpt, $rettpt]
      ...
      q"""
        def $$enter(
          ...
          val pc = _root_.com.storm_enroute.coroutines.common.Stack.top(c.$$pcstack)
      ...
      q"""
        def $$enter(
          ...
          val pc: Short = _root_.com.storm_enroute.coroutines.common.Stack.top(c.$$pcstack)
  ...
  private def genReturnValueMethod(cfg: Cfg, tpe: Type)(implicit table: Table): Tree = {
    ...
    q"""
      def $returnvaluemethod(
        c: _root_.com.storm_enroute.coroutines.Coroutine.Instance[
  ...
  def genVarPushesAndPops(cfg: Cfg)(implicit table: Table): (List[Tree], List[Tree]) = {
    ...
    def genVarPushes(allvars: Map[Symbol, VarInfo], stack: Tree): List[Tree] = {
      ...
      val bulkpushes = if (vars.size == 0) Nil else List(q"""
        _root_.com.storm_enroute.coroutines.common.Stack.bulkPush($stack, $varsize, $stacksize)
      """)
    ...
    val varpops = (for ((sym, info) <- storedRefVars.toList) yield {
      ...
      q"""
        _root_.com.storm_enroute.coroutines.common.Stack.bulkPop(
  ...
  def specArity1(
    ...
    if (tpe == typeOf[scala.Boolean]) {
      (tq"com.storm_enroute.coroutines.Coroutine._1", argtpts :+ yldtpt :+ rettpt)
    } else if (tpe == typeOf[scala.Byte]) {
      (tq"com.storm_enroute.coroutines.Coroutine._1", argtpts :+ yldtpt :+ rettpt)
    } else if (tpe == typeOf[scala.Short]) {
      val nme = TypeName(s"_1$$spec$$S")
      (tq"com.storm_enroute.coroutines.$nme", yldtpt :: rettpt :: Nil)
      ...
      (tq"_root_.com.storm_enroute.coroutines.$nme", yldtpt :: rettpt :: Nil)
    } else {
      val nme = TypeName(s"_1$$spec$$L")
      (tq"_root_.com.storm_enroute.coroutines.$nme", argtpts :+ yldtpt :+ rettpt)
    }
  }
  ...
  def specArity2(
    ...
    if (tp0 == typeOf[scala.Int] && tp1 == typeOf[scala.Int]) {
      ...
      (tq"_root_.com.storm_enroute.coroutines.$nme", argtpts(0) :: yldtpt :: rettpt :: Nil)
      ...
      (tq"_root_.com.storm_enroute.coroutines.$nme", argtpts(1) :: yldtpt :: rettpt :: Nil)
      ...
      (tq"_root_.com.storm_enroute.coroutines.$nme", tpes)
    }
  }
  ...
  def genCoroutineTpe(
    ...
    if (argtpts.length == 1) {
      ...
      (tq"_root_.com.storm_enroute.coroutines.Coroutine.$nme", argtpts :+ yldtpt :+ rettpt)
  ...
  def synthesize(rawlambda: Tree): Tree = {
    ...
    val co = q"""
      new $coroutinequal[..$tparams] {
        def $$call(
          ..$args
        ): _root_.com.storm_enroute.coroutines.Coroutine.Instance[$yldtpt, $rettpt] = {
          val $valnme = new _root_.com.storm_enroute.coroutines.Coroutine.Instance[$yldtpt, $rettpt]
          $$push($valnme, ..$argidents)
          $valnme
        }
        def apply(..$args): $rettpt = {
          _root_.scala.sys.error(
            _root_.com.storm_enroute.coroutines.COROUTINE_DIRECT_APPLY_ERROR_MESSAGE)
        }
        def $$push(
          $$c: _root_.com.storm_enroute.coroutines.Coroutine.Instance[$yldtpt, $rettpt], ..$args
        ): _root_.scala.Unit = {
          _root_.com.storm_enroute.coroutines.common.Stack.push($$c.$$costack, this, -1)
          _root_.com.storm_enroute.coroutines.common.Stack.push($$c.$$pcstack, 0.toShort, -1)
          ..$varpushes
        }
        def $$pop(
          $$c: _root_.com.storm_enroute.coroutines.Coroutine.Instance[$yldtpt, $rettpt]
        ): _root_.scala.Unit = {
          _root_.com.storm_enroute.coroutines.common.Stack.pop($$c.$$pcstack)
          _root_.com.storm_enroute.coroutines.common.Stack.pop($$c.$$costack)
          ..$varpops
        }
        $entermethod
        ..$entrypointmethods
